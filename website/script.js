    ...
    if (searchChefsBtn) {
        searchChefsBtn.addEventListener('click', function() {
            ...
            chefCards.forEach(card => {
                ...
                // Location filter - all chefs are in California for demo
                if (location && location !== 'california' && location !== 'ca' && location !== 'san jose' && location !== '') {
                    showCard = false; // Hide if location doesn't match California area
                }
            ...
            // Optional: Show results summary (you can remove this if not needed)
            console.log(`Search completed! Found ${visibleCount} chef(s) matching: Location: ${location || 'Any'}, Cuisine: ${cuisine || 'Any'}, Dietary: ${dietary || 'Any'}`);
        });
    }
});
